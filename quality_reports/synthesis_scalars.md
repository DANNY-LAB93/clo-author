# Cifras del manuscrito

Generado por `scripts/build_synthesis_scalars.py`. **Ninguna cifra del
manuscrito se teclea a mano: se cita por su nombre desde esta tabla.**

| Nombre | Valor |
|---|---|
| `fuentes_n` | 9 |
| `fuentes_nombres` | BVS (no MEDLINE), CTIS, ClinicalTrials.gov, Cochrane CENTRAL, EudraCT, PubMed, SciELO, Scopus (brazo A), Scopus (brazo B) |
| `registros_por_fuente` | PubMed: 9561; Scopus (brazo B): 9360; Scopus (brazo A): 2871; BVS (no MEDLINE): 575; SciELO: 241; Cochrane CENTRAL: 225; ClinicalTrials.gov: 121; CTIS: 10; EudraCT: 3 |
| `informes_unicos` | 17129 |
| `registros_identificados` | 23057 |
| `duplicados_eliminados` | 5928 |
| `excluidos_etapa1` | 3235 |
| `cribados_por_titulo` | 13894 |
| `excluidos_titulo` | 13434 |
| `a_resumen` | 460 |
| `excluidos_resumen` | 192 |
| `informes_a_texto_completo` | 268 |
| `corriente_bases` | 389 |
| `corriente_registros` | 71 |
| `estudios` | 219 |
| `informes_agrupados` | 268 |
| `estudios_con_articulo` | 156 |
| `estudios_solo_resumen` | 3 |
| `estudios_solo_registro` | 60 |
| `estudios_multiinforme` | 30 |
| `informes_del_estudio_mayor` | 11 |
| `estudios_extraibles` | 159 |
| `texto_completo_obtenido` | 74 |
| `texto_completo_no_obtenido` | 85 |
| `texto_completo_pct` | 46.5 |
| `disenos` | case report: 52; prospective cohort: 22; case series: 21; RCT: 21; non-randomised trial: 20; no declarado: 18; retrospective cohort: 5 |
| `estudios_comparativos` | 41 |
| `estudios_comparativos_pct` | 25.8 |
| `ecas` | 21 |
| `casos_unicos` | 52 |
| `casos_unicos_pct` | 32.7 |
| `comparativos_sin_texto` | 26 |
| `comparativos_sin_texto_pct` | 63.4 |
| `pacientes_declarados_con_texto` | 1839 |
| `pacientes_declarados_sin_texto` | 2094 |
| `procedencia` | no declarada: 81; Rusia: 34; Polonia: 6; Georgia: 6; Alemania: 4; Estados Unidos: 3; Francia: 3; Ucrania: 3; Israel: 2; India: 2 |
| `procedencia_no_declarada` | 81 |
| `procedencia_no_declarada_pct` | 50.9 |
| `anio_min` | 2016 |
| `anio_max` | 2026 |
| `publicados_desde_2020` | 130 |
| `publicados_desde_2020_pct` | 81.8 |
| `sin_ambito_de_patogeno` | 97 |
| `sin_ambito_de_patogeno_pct` | 61.0 |
| `sin_clase_de_resistencia` | 122 |
| `sin_clase_de_resistencia_pct` | 76.7 |
| `sin_via_de_administracion` | 96 |
| `sin_via_de_administracion_pct` | 60.4 |
| `sin_modalidad` | 120 |
| `sin_modalidad_pct` | 75.5 |
| `sin_criterio_dtr` | 157 |
| `sin_criterio_dtr_pct` | 98.7 |
| `exclusiones_titulo` | ORG: 3031; REV: 2807; SEC: 84; LAB: 4062; VET: 838; OFF: 2634 |
| `exclusiones_resumen` | ORG: 65; REV: 64; SEC: 5; LAB: 18; OFF: 40 |
